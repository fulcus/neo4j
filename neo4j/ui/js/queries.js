...
$("#submit_lives").click(function () {
  var cypher =
    ...
    "', email:'" +
    $("#email1").val() +
    "'})-[r:LIVES_WITH]->(p2:Person{first_name:'" +
    $("#first_name2").val() +
    "', last_name:'" +
    $("#last_name2").val() +
    "', cf:'" +
...
$("#submit_vaccinated").click(function () {
  var cypher =
    ...
    "', email:'" +
    $("#emailr").val() +
    "'})-[r:VACCINATED_WITH]->(p2:Vaccine{date:'" +
    $("#vaccine_dater").val() +
    "', type:'" +
    $("#vaccine_typer").val() +
    "'}) RETURN p1,p2,r";
...
$("#q1").click(function () {
  var cypher =
    "MATCH (t:Test{outcome:'true'})<-[r:TESTED_WITH]-(p:Person)-[v:VISITED]->(q:Place) WHERE date(t.date) >= date(v.datetime) AND date(t.date) <= date(v.datetime) + duration('P14D') RETURN q, p, v, t, r";
  viz.renderWithCypher(cypher);
});

$("#q2").click(function () {
  var cypher =
    "MATCH (p)-[r:LIVES_WITH]-(p1:Person)-[t:TESTED_WITH]->(t1:Test{outcome:'true'}), (t2:Test{outcome:'true'})WHERE date(t1.date) <= date(t2.date) AND date(t1.date)+duration('P10D') >= date(t2.date)AND NOT (p)-[:TESTED_WITH]->(t2)RETURN DISTINCT p,p1,r,t,t1";
  viz.renderWithCypher(cypher);
});
...
$("#q4").click(function () {
  var cypher =
    "MATCH (t:Test{outcome:'true'})<-[r:TESTED_WITH]-(p:Person)-[v:VISITED{datetime:datetime('2021-08-15')}]->(q:Place{type:'Bar'})WHERE date(t.date) <= date(v.datetime)+duration('P14D') AND date(t.date) >= date(v.datetime)RETURN p,v,q,r,t";
  viz.renderWithCypher(cypher);
});
